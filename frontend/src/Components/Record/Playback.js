...
export function playMidiFile(uri, instArr, NTArr, bpm)
{
    ...
    Player.on('midiEvent', function(event) {
        if (event.noteNumber != 0 && event.noteNumber != undefined && event.name !== "Note off") //eventCount % 2 == 0) // As long as the midiEvent is a valid NOTE (other events not supported) and not undefined
        {
            ...
            playMidiNote(midiNoteFrequency, defaultVolume + volumeDeltas[event.track - 1], instArr[event.track - 1], getNoteTypeStringFromInt(NTArr[event.track - 1]));
        }
        eventCount++;
    });
}
...
function getNoteTypeStringFromInt(input)
{
